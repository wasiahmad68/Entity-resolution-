...
def render_record_views(records):
            ...
            if "FEATURES" in rec and isinstance(rec["FEATURES"], list):
                for f_obj in rec["FEATURES"]:
                    for k, v in f_obj.items():
                        # We append the key to the row so it becomes a neat column
                        # If a key already exists, we could join them with a comma
                        if k in row:
                            row[k] = f"{row[k]} | {v}"
                        else:
                            row[k] = v
    ...
    with t3:
        ...
        if len(ordered_records) > 1:
            # Create side-by-side columns based on the number of records (max 4 to avoid crowding, wrap otherwise)
...
{"DATA_SOURCE": "SOURCE_NAME", "RECORD_ID": "UNIQUE_ID_124", "FEATURES": [{"NAME_FIRST": "Jane"}]}''', language="json")
            ...
            try:
                ...
                try:
                    ...
                    if not isinstance(data, list):
                        data = [data] # Fallback if they uploaded a single object accidentally
                except json.JSONDecodeError:
                    # Fallback to JSON Lines (JSONL) parsing
                    data = []
    ...
    with tab_management:
        ...
        tab1, tab2, tab_duplicates, tab3 = st.tabs(["Single Ingestion", "Fetch Raw Record", "Find Duplicate Records", "Delete Record"])
        
        with tab1:
            st.subheader("Upsert Single Record")
            ds_val = "" if st.session_state["active_ds"] == "ALL" else st.session_state["active_ds"]
            ds = st.text_input("Data Source", value=ds_val, disabled=(st.session_state["active_ds"] != "ALL"), key="ingest_ds")
            rid = st.text_input("Record ID", key="ingest_rid")
            payload_str = st.text_area("JSON Payload (Must contain FEATURES list)", height=200, value='{"FEATURES": [{"NAME_FIRST": "John", "NAME_LAST": "Doe"}]}')
        ...
        with tab2:
            ...
            f_ds_val = "" if st.session_state["active_ds"] == "ALL" else st.session_state["active_ds"]
            f_ds = st.text_input("Data Source", value=f_ds_val, disabled=(st.session_state["active_ds"] != "ALL"), key="fetch_ds")
            
            f_mode = st.radio("Fetch Input Method", ["Manual Input (Comma-Separated)", "File Upload (CSV/JSON/JSONL)"], key="fetch_radio")
            
            rids = []
            if "Manual" in f_mode:
                f_rids_str = st.text_area("Record IDs (comma-separated)", key="fetch_rid", placeholder="123, 456, 789")
                if f_rids_str:
                    rids = [rid.strip() for rid in f_rids_str.split(",") if rid.strip()]
            else:
                f_file = st.file_uploader("Upload file with `record_id` or `RECORD_ID` column/keys", type=["csv", "json", "jsonl"], key="fetch_file")
                if f_file:
                    try:
                        if f_file.name.endswith(".csv"):
                            ...
                            if isinstance(f_data, list):
                                rids = [str(item.get("record_id", item.get("RECORD_ID"))) for item in f_data if item.get("record_id") or item.get("RECORD_ID")]
                        elif f_file.name.endswith(".jsonl"):
                            content = f_file.getvalue().decode("utf-8")
                            for line in content.splitlines():
                                if line.strip():
                                    item = json.loads(line)
                                    rid = item.get("record_id", item.get("RECORD_ID"))
                                    if rid: rids.append(str(rid))
                    except Exception as e:
            ...
            if st.button("Fetch"):
                if not rids:
                    ...
                    not_found = []
                    for rid in rids:
                        res = engine.get_raw_record(f_ds, rid)
                        if res:
        ...
        with tab_duplicates:
            ...
            dup_ds_val = "" if st.session_state["active_ds"] == "ALL" else st.session_state["active_ds"]
            dup_ds = st.text_input("Data Source", value=dup_ds_val, disabled=(st.session_state["active_ds"] != "ALL"), key="dup_ds")
            
            dup_mode = st.radio("Search Input Method", ["Manual Input (Comma-Separated)", "File Upload (CSV/JSON/JSONL)"], key="dup_radio")
            
            dup_rids = []
            if "Manual" in dup_mode:
                dup_rids_str = st.text_area("Record IDs (comma-separated)", key="dup_rid", placeholder="123, 456, 789")
                if dup_rids_str:
                    dup_rids = [rid.strip() for rid in dup_rids_str.split(",") if rid.strip()]
            else:
                dup_file = st.file_uploader("Upload file with `record_id` or `RECORD_ID` column/keys", type=["csv", "json", "jsonl"], key="dup_file")
                if dup_file:
                    try:
                        if dup_file.name.endswith(".csv"):
                            ...
                            if isinstance(dup_data, list):
                                dup_rids = [str(item.get("record_id", item.get("RECORD_ID"))) for item in dup_data if item.get("record_id") or item.get("RECORD_ID")]
                            ...
                            for line in content.splitlines():
                                if line.strip():
                                    item = json.loads(line)
                                    rid = item.get("record_id", item.get("RECORD_ID"))
                                    if rid: dup_rids.append(str(rid))
                    except Exception as e:
                        st.error(f"Failed to read file: {e}")
        ...
        with tab3:
            ...
            d_ds_val = "" if st.session_state["active_ds"] == "ALL" else st.session_state["active_ds"]
            d_ds = st.text_input("Data Source", value=d_ds_val, disabled=(st.session_state["active_ds"] != "ALL"), key="del_ds", help="Target Data Source for these deletions.")
            
            del_mode = st.radio("Deletion Input Method", ["Manual Input (Comma-Separated)", "File Upload (CSV/JSON/JSONL)"])
            
            record_ids_to_del = []
            if "Manual" in del_mode:
                d_rids_str = st.text_area("Record IDs (comma-separated)", key="del_rid_manual", placeholder="123, 456, 789")
                if d_rids_str:
                    record_ids_to_del = [rid.strip() for rid in d_rids_str.split(",") if rid.strip()]
            else:
                del_file = st.file_uploader("Upload file with `record_id` or `RECORD_ID` column/keys", type=["csv", "json", "jsonl"])
                if del_file:
                    try:
                        if del_file.name.endswith(".csv"):
                            ...
                            if isinstance(del_data, list):
                                record_ids_to_del = [str(item.get("record_id", item.get("RECORD_ID"))) for item in del_data if item.get("record_id") or item.get("RECORD_ID")]
                            ...
                            for line in content.splitlines():
                                if line.strip():
                                    item = json.loads(line)
                                    rid = item.get("record_id", item.get("RECORD_ID"))
                                    if rid: record_ids_to_del.append(str(rid))
                    except Exception as e:
                        st.error(f"Failed to read file: {e}")
    ...
    with tab_search:
        ...
        # Explicit Attribute selector logic (up to 5)
        ...
        # Prepend an empty option so they don't have to fill all 5
        ...
        if len(available_attrs) <= 1:
            ...
            if submitted_attr:
                # Extract valid (Attribute, Value) pairs where both are provided
    ...
    with tab_graph:
        ...
        g_ds_val = "" if st.session_state["active_ds"] == "ALL" else st.session_state["active_ds"]
        datasource = st.text_input("Data Source (e.g. SYSTEM_A)", value=g_ds_val, disabled=(st.session_state["active_ds"] != "ALL"))
        ...
        if st.session_state.get("graph_ds") and st.session_state.get("graph_rid"):
            ...
            if "error" in analysis:
                ...
                if agraph and Node and Edge:
                    ...
                    # Cache nodes and edges so graph layout doesn't reset when clicking
                    if state_key not in st.session_state:
                        ...
                        nodes.append(Node(id=f"ENT_{entity_id}", 
                        ...
                        for rec in analysis.get('cluster_records', []):
                            ...
                            nodes.append(Node(id=record_node_id, 
                            ...
                            edges.append(Edge(source=record_node_id, 
                                              target=f"ENT_{entity_id}", 
                                              label=f"[Lvl 1] {rec.get('rule_fired','EXACT')}",
                                              color="#A8DADC",
                                              length=200))
                                              
                        # Level 2/3 Related Entities
                        for rel in analysis.get('relationships', []):
                            r_name = rel.get('rule_fired', 'RELATED')
                            r_level = rule_lvl_map.get(r_name, "2/3")
                            ...
                            for rr in rel_records:
                                ...
                                nodes.append(Node(id=rel_record_id,
                                ...
                                edges.append(Edge(source=f"ENT_{entity_id}",
    ...
    with tab_config:
        ...
        tab_rules, tab_sources, tab_attributes = st.tabs(["Match Rules", "Data Source Whitelisting", "Indexed Attributes"])
        ...
        with tab_rules:
            ...
            with st.expander("Inject Custom Deterministic Rule"):
                with st.form("custom_rule_form"):
                    ...
                    # Pull existing schemas for multiselect
                    ...
                    rule_level = st.selectbox("Resolution Level", [1, 2, 3], help="Level 1 = Identical Entity Merge, 2/3 = Distinct Relationships")
